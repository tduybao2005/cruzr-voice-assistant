...
def test_chunk_render_prefixes_id():
    c = Chunk(id="menu:ML_LATTE", text="Giá 55.000đ", source="test")
    assert c.render() == "[menu:ML_LATTE]\nGiá 55.000đ"
...
def test_faq_chunk_keeps_question_and_answer_together():
    chunks = chunk_faq_file(config.KB_DIR / "05_Bo_Cau_Hoi_Thuong_Gap_FAQ.md")
    latte = next(c for c in chunks if "Mộc Lam Latte có vị như thế nào" in c.text)
...
def test_every_chunk_carries_document_title_for_context():
    ...
    assert all("MỘC LAM" in c.text.upper() for c in chunks)
...
def test_subsection_chunks_keep_parent_section_context():
    ...
    assert "MỘC LAM" in coffee.text.upper(), "chunk con vẫn phải mang tên quán"
