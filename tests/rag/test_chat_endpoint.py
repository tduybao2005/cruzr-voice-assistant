...
def test_retrieve_endpoint_reports_scores(client):
    api.retriever = FakeRetriever(
        RetrievalResult(
            chunks=[RetrievedChunk("menu:ML_LATTE_M", "Giá 55,000 VNĐ", 0.82)],
            in_scope=True,
            top_score=0.82,
        )
    )
    resp = client.post("/retrieve", json={"query": "Mộc Lam Latte giá bao nhiêu"})
    body = resp.json()
    assert body["in_scope"] is True
    assert body["chunks"][0]["chunk_id"] == "menu:ML_LATTE_M"
...
def test_top_k_from_the_request_reaches_the_retriever(client):
    ...
    client.post("/retrieve", json={"query": "Mộc Lam Latte giá bao nhiêu", "top_k": 7})
    assert fake.top_k_seen == 7

    client.post("/chat", json={"message": "Mộc Lam Latte giá bao nhiêu", "top_k": 5})
