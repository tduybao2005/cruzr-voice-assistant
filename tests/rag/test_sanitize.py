...
def test_drops_source_ids_not_in_context():
    raw = json.dumps({"intent": "MENU_QA", "answerText": "x",
                      "sourceIds": ["menu:ML_LATTE_M", "menu:KHONG_CO_THAT"]})
    out = json.loads(sanitize_response(raw, ["menu:ML_LATTE_M", "faq:md_001"]))
    assert out["sourceIds"] == ["menu:ML_LATTE_M"]


def test_repairs_id_missing_its_prefix():
    """Base model hay trả 'ML_TIRAMISU' thay vì 'menu:ML_TIRAMISU'."""
    raw = json.dumps({"sourceIds": ["ML_TIRAMISU", "md_001"]})
    out = json.loads(sanitize_response(raw, ["menu:ML_TIRAMISU", "faq:md_001"]))
    assert out["sourceIds"] == ["menu:ML_TIRAMISU", "faq:md_001"]


def test_keeps_order_and_drops_duplicates():
    raw = json.dumps({"sourceIds": ["faq:md_001", "menu:ML_LATTE_M", "faq:md_001"]})
    out = json.loads(sanitize_response(raw, ["menu:ML_LATTE_M", "faq:md_001"]))
    assert out["sourceIds"] == ["faq:md_001", "menu:ML_LATTE_M"]
...
def test_other_fields_are_preserved_exactly():
    ...
    out = json.loads(sanitize_response(raw, ["menu:ML_PEACH_TEA"]))
