...
def test_parse_chunk_id_strips_the_id_line():
    cid, body = parse_chunk_id("[menu:ML_LATTE]\nGiá 55.000đ")
    assert cid == "menu:ML_LATTE"
...
def test_scores_below_threshold_are_dropped_and_marked_out_of_scope():
    with patch("cadebot.rag.retriever.requests.post",
               return_value=_fake_response([_record("[menu:ML_LATTE]\nx", 0.21)])):
...
def test_scores_above_threshold_are_kept_and_marked_in_scope():
    with patch("cadebot.rag.retriever.requests.post", return_value=_fake_response([
        _record("[menu:ML_LATTE]\nGiá 55.000đ", 0.82),
        _record("[faq:md_001]\nQ: vị thế nào\nA: béo nhẹ", 0.61),
        _record("[doc:03_Khong_Gian#1]\nkhông liên quan", 0.30),
    ])):
        result = Retriever(threshold=0.55).retrieve("Mộc Lam Latte giá bao nhiêu")
    assert result.in_scope is True
    assert result.source_ids == ["menu:ML_LATTE", "faq:md_001"]
...
def test_per_request_top_k_overrides_the_default():
    ...
    with patch("cadebot.rag.retriever.requests.post",
        ...
        Retriever(top_k=3).retrieve("Mộc Lam Latte giá bao nhiêu", top_k=7)
...
def test_omitting_top_k_falls_back_to_the_instance_default():
    with patch("cadebot.rag.retriever.requests.post",
               return_value=_fake_response([])) as post:
        Retriever(top_k=3).retrieve("Mộc Lam Latte giá bao nhiêu")
