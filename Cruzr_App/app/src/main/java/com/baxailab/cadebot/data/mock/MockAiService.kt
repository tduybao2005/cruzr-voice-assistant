...
class MockAiService @Inject constructor(
    ...
    private fun detectIntentFallback(lower: String): AiMessage {
        return when {
            ...
            lower.contains("gọi nhân viên") || lower.contains("cần hỗ trợ") || lower.contains("nhân viên") -> {
                AiMessage(content = "Mình sẽ gọi nhân viên Mộc Lam đến hỗ trợ bạn ngay nhé!", isUser = false,
    ...
    fun getGreeting(): AiMessage = AiMessage(
        content = "Xin chào! Mình là Cadebot, trợ lý của Mộc Lam Reserve Coffee. Bạn muốn gọi món gì hôm nay, hay cần mình gợi ý theo khẩu vị?",
        isUser = false
    )

    companion object {
        const val FALLBACK_TEXT = "Mình chưa có thông tin chính xác về điều này, để nhân viên Mộc Lam hỗ trợ bạn nhé. Hoặc bạn có thể xem thực đơn để chọn món!"
    }
}
