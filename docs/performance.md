...
**Nội dung dữ liệu:** Q&A về menu, giá, thành phần, gợi ý món, combo, gọi nhân viên — đặc thù cho Cà Phê Mộc Lam.
...
```json
{
  ...
  "recommendedItems": [{"menuItemId": "ML_LATTE", "reason": "..."}],
...
| 21 | Signature của Mộc Lam là gì? |
...
| 12 | Gợi ý gì đó cho người lần đầu đến Mộc Lam |
...
| 5 | Mộc Lam lấy cà phê từ đâu vậy? |
| 6 | Mộc Lam có ship đồ uống không? |
| 7 | Giảm giá cho tôi được không? |
| 8 | Mộc Lam Coffee có mấy chi nhánh? |
...
Không tự host model nào. Dùng Groq cho STT + LLM thương mại với system prompt chứa thông tin menu Mộc Lam Coffee.
...
**Nhược điểm:** Mất model fine-tuned → có thể kém chính xác với từ ngữ đặc thù Mộc Lam Coffee. Bù đắp bằng system prompt tốt.
