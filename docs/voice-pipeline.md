...
|---|---|---|---|
| `cadebot-moclam:latest` | 1d704a6609bc | **3.3 GB** | Qwen2.5-3B fine-tuned cho Cà Phê Mộc Lam (Q8_0) |
| `qwen2.5:3b` | 357c53fb659c | 1.9 GB | Base model gốc (Q4) |

### Modelfile của cadebot-moclam

```
FROM ./cadebot-moclam.gguf
SYSTEM "Bạn là Cadebot, trợ lý robot phục vụ tại Cà Phê Mộc Lam..."
...
| `pipeline/llm.py` | Gọi Ollama API (`cadebot-moclam`) → parse JSON response |
...
# Kiểm tra Ollama + model cadebot-moclam có sẵn
python3 test_pipeline.py --module ollama --model cadebot-moclam

# Test LLM — 6 kịch bản intent (MENU_QA, ADD_TO_CART, REC, FALLBACK, CALL_STAFF, PROMO)
python3 test_pipeline.py --module llm --model cadebot-moclam
...
curl -s http://localhost:11434/api/chat \
  -d '{"model":"cadebot-moclam","messages":[{"role":"user","content":"Latte giá bao nhiêu?"}],"stream":false}' \
