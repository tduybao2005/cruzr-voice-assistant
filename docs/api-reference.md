...
```json
{"text": "Mộc Lam Latte giá bao nhiêu"}
```
...
```json
{
  "message": "Mộc Lam Latte giá bao nhiêu",
  "history": [
...
```json
{
  "response": "{\"intent\":\"MENU_QA\",\"confidence\":0.95,\"answerText\":\"Mộc Lam Latte có giá 55.000 VNĐ bạn nhé!\",\"spokenText\":\"Mộc Lam Latte giá 55 nghìn đồng.\",\"recommendedItems\":[],\"draftCartItems\":[],\"requiresHumanSupport\":false,\"sourceIds\":[\"menu:ML_LATTE_M\"]}",
  "retrieval": {
    "in_scope": true,
    "top_score": 0.82,
    "threshold": 0.51,
...
model invented are dropped, and IDs that lost their prefix (`ML_TIRAMISU`) are
repaired to the full form (`menu:ML_TIRAMISU`). Chunk ID namespaces are
...
```json
{
  "response": "{\"intent\":\"FALLBACK\",\"confidence\":1.0,\"answerText\":\"Xin lỗi bạn, mình chưa có thông tin chính xác về điều này. Bạn vui lòng hỏi nhân viên Mộc Lam để được hỗ trợ nhé!\",\"spokenText\":\"Xin lỗi bạn, mình chưa có thông tin chính xác về điều này. Bạn vui lòng hỏi nhân viên Mộc Lam để được hỗ trợ nhé!\",\"recommendedItems\":[],\"draftCartItems\":[],\"requiresHumanSupport\":true,\"sourceIds\":[]}",
...
curl -s -X POST localhost:8000/retrieve \
  -H 'Content-Type: application/json' \
  -d '{"query":"Mộc Lam Latte giá bao nhiêu"}' | python3 -m json.tool
```
...
```json
{
    ...
    "chunks": [
        {
            "chunk_id": "menu:ML_LATTE_M",
            "score": 0.82,
            "text": "Món: Mộc Lam Latte (Mã: ML_LATTE_M | Nhóm: Cà Phê)\n  + Giá: 55,000 VNĐ..."
        }
    ]
}
```
